...
# Path folder utama
folder_utama = 'Praproses\output_image'

# Mengambil nama subfolder (kelas)
kelas = os.listdir(folder_utama)

# Inisialisasi variabel untuk menyimpan histogram dan bins
histograms = []
bins = np.arange(256)
...
for k in kelas:
    # Path subfolder
    path_kelas = os.path.join(folder_utama, k)

    # Mengambil path semua gambar dalam subfolder
    paths = [os.path.join(path_kelas, f) for f in os.listdir(path_kelas) if f.endswith('.jpg')]

    # Loop untuk menghitung histogram dari setiap gambar dalam subfolder
    for path in paths:
        image = cv.imread(path)
        hist, _ = np.histogram(image.ravel(), bins=[0, 256], range=[0, 256])
        hist_normalized = hist / np.sum(hist)
        histograms.append(hist_normalized)

# Menggabungkan data menjadi satu matriks train
dataLatih = np.vstack(histograms).astype(np.float32)

# Memberi label
label_mapping = {'MENTAH': 0, 'MENUJU MATANG': 1, 'MATANG': 2, 'MENUJU BUSUK': 3, 'BUSUK': 4}

label = np.concatenate([np.full(len(os.listdir(os.path.join(folder_utama, k))), label_mapping[k]) for k in kelas]).astype(np.float32)
...
for train, test in skf.split(dataLatih, label):
    ...
    y_pred = knn.predict(X_uji)
    print(accuracy_score(Y_uji, y_pred))
    print(confusion_matrix(Y_uji, y_pred, labels=np.unique(label)))

# Menghitung akurasi dengan cross_val_score
akurasi = cross_val_score(knn, dataLatih, label, cv=4, scoring='accuracy')
print('Akurasi :', akurasi.mean())
